...
class FileCache:
    ...
    def _perform_write(self, file_path: str, data: Dict[str, Any], write_type: str = "sync"):
        ...
        try:
            ...
            print(f"Write error for {file_path}: {e}")
    ...
    def _is_cache_valid(self, file_path: str) -> bool:
        ...
        age = time.time() - self._cache_timestamps[file_path]
        return age < self._cache_ttl
    
    def _is_file_changed(self, file_path: str) -> bool:
        """Check if file has changed on disk using file modification time"""
        if file_path not in self._file_hashes:
            return True

        try:
            # Use file modification time instead of reading content
    ...
    def read_json(self, file_path: str, default: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            try:
                ...
                self._cache[file_path] = data.copy()
                self._cache_timestamps[file_path] = os.path.getmtime(file_path) if os.path.exists(file_path) else time.time()
                self._file_hashes[file_path] = self._calculate_hash(data)
