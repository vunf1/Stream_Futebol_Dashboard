...
class PathFinder:
    ...
    def resource(self, *relative: str | os.PathLike) -> Path:
        ...
        """
        rel_path = Path(*map(str, relative))
        if self._frozen and self._meipass:
            cand = Path(self._meipass) / rel_path
            if cand.exists():
                return cand
        # Dev / fallback
        return self._root / rel_path

    def user_local_appdir(self, *relative: str | os.PathLike) -> Path:
